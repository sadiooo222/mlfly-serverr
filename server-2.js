...
async function freeResolveAirport(keyword) {
  ...
  const scored = [];
  for (const a of db) {
    const name = norm(a.name), city = norm(a.city_name), code = norm(a.code);
    let score = -1;
    if (code === q) score = 100;
  ...
  if (!scored.length || scored[0].score < 90) {
    try {
      ...
      if (match) {
        const inCountry = db.filter(a => a.country_code === match.code && a.flightable !== false);
        inCountry.sort((x, y) => (y.time_zone ? 1 : 0) - (x.time_zone ? 1 : 0));
        inCountry.slice(0, 8).forEach(a => scored.push({ score: 70, a }));
      }
    } catch (e) { /* pas grave si indispo, on garde les résultats déjà trouvés */ }
  }
  scored.sort((x, y) => y.score - x.score);
  const seen = new Set();
  return scored.filter(({ a }) => !seen.has(a.code) && seen.add(a.code)).slice(0, 8).map(({ a }) => ({
    iata: a.code, name: a.name, city: a.city_name, country: a.country_code, type: 'AIRPORT'
  }));
}
